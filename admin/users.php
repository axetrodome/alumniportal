...
$perPage = !empty(Input::get('per-page')) && Input::get('per-page') <= 50 ? (int)Input::get('per-page') : 25;
// numbering
...
<body>
		...
		<div class="container">
			<h3>Users</h3>
			<?php
				$user->page('users',array($start,$perPage));
				$total = $user->total();
				...
				<?php 	foreach($user->DBresults() as $users): ?>
						<tbody>
							<tr id="search">
								<td><?= $y; ?></td>
